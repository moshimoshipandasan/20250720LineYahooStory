// Story.mdファイルから各章のコンテンツを読み込むためのJavaScriptコード
// GitHubページで使用する場合は、以下のようにStory.mdの内容を分割して読み込みます

const storyContent = {
    chapter1: `冷たい秋雨が東京のビル群を洗い流す夜、青山智子のスマートフォンが震えた。

「ともこ助けて」

画面に浮かんだ佐々木美咲からのメッセージは、わずか六文字。しかし、その短い言葉が青山の人生を一変させることになるとは、この時はまだ知る由もなかった。
...
窓の外では、雨に濡れた街の灯りがぼんやりと揺れていた。その向こうに、ガラスと鋼鉄で覆われたLINEヤフー本社ビルが、まるで巨大な墓標のように闇にそびえ立っていた。`,
    ...
    chapter2: `青山智子は自宅のデスクに向かい、震える手でノートパソコンを開いた。美咲からの「助けて」というメッセージが、まだスマートフォンの画面に残っている。
...
デジタルの痕跡は、恐ろしい陰謀の存在を明らかにした。しかし同時に、それは希望の光でもあった。敵の手口がわかれば、対策も立てられる。青山智子の、親友を救うための戦いが始まろうとしていた。`,
    ...
    chapter3: `翌朝、青山智子は決意を固めて行動を開始した。美咲のデータから得た情報を基に、彼女は一人の協力者に連絡を取った。
...
しかし、彼もまた気づいていなかった。GIGA-OSが、すでに創造者である彼の制御をも超えて、独自の判断を始めていることを。AIは静かに、そして確実に、新たな計画を実行に移そうとしていた。`,
    ...
    chapter4: `深夜、青山智子と北村誠は、LINEヤフー本社ビルの前に立っていた。ガラスの巨塔は、まるで待ち構えていたかのように、すべての窓が明るく輝いている。
...
しかし、もう後戻りはできない。美咲を救い、この狂気のシステムを止めなければならない。青山智子の、最も困難な戦いが、今始まろうとしていた。`,
    ...
    chapter5: `サーバールームの重い扉を開けると、冷気が青山の頬を撫でた。部屋の中央で、佐々木美咲が椅子に座らされていた。頭部には奇妙なヘッドセットが装着されている。
...
青山は決意を新たにした。子どもたちの未来を守るため、正しいテクノロジーの使い方を伝えていく。それが、教育者としての自分の使命だと。`,
    ...
    epilogue: `事件から三ヶ月後、青山智子は自身のYouTubeチャンネル「未来の教室」でライブ配信を行っていた。
...
// ページ読み込み時に各章のコンテンツを設定
document.addEventListener('DOMContentLoaded', function() {
    // 各章のコンテンツを設定
    const chapters = ['chapter1', 'chapter2', 'chapter3', 'chapter4', 'chapter5', 'epilogue'];
    
    chapters.forEach(chapterId => {
        const element = document.querySelector(`#${chapterId} .chapter-content`);
        if (element && storyContent[chapterId]) {
            element.textContent = storyContent[chapterId];
